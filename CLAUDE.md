...
Chat z użytkownikiem po polsku. Kod, identyfikatory, komentarze, commit messages — po angielsku.

## Critical "DO NOT" rules

- **Do not hand-edit anything between the BEGIN / END HTML comment markers below.** That section is auto-managed by `@przeprogramowani/10x-cli` and overwritten on every `10x get mNlN`. Write project guidance above the markers, never inside them. **Also do not quote those marker strings verbatim anywhere in this file** — the CLI parses them as sentinels and will nuke everything between the first match and the next END.
- **Do not write into `context/archive/`.** Archived changes are immutable. Skills refuse this path. If a target resolves under `context/archive/`, open a new change in `context/changes/` instead.
- **Do not `--no-verify`** when the pre-commit gate fails. Fix the source — `husky` + `lint-staged` config in @package.json.
- **Do not remove `package.json` `overrides`** (`yaml: ^2.8.3`, `ws: ^8.20.1`) without re-running `npm audit`. They close transitive vulns surfaced post-bootstrap.
- **Do not assume `.git/` exists.** Repo not yet `git init`-ed. Check before any `git` command.

## Database types must be generated first

`src/middleware.ts`, `src/lib/supabase.ts`, and `src/pages/auth/confirm-email.astro` ship with 20 `@typescript-eslint/no-unsafe-*` errors because `@supabase/ssr` returns `any` in generic mode. Generate the typed `Database` schema and parameterize the client to fix them in one move:

```
npx supabase gen types typescript --local > src/db/database.types.ts
```

Then change `createServerClient(...)` to `createServerClient<Database>(...)` in `src/lib/supabase.ts` and import the type at call sites. Without this, the pre-commit gate blocks most commits.

## Conventions worth knowing

- **API routes** (`src/pages/api/*`): **Zod validation at the request boundary**.
- **React Compiler is strict**: `react-compiler/react-compiler` is set to `error` in `eslint.config.js`. Hook-rule violations BLOCK lint — not warnings.
- **UI primitives**: shadcn/ui in **"new-york"** style. Use `cn()` from `src/lib/utils` for conditional classes — never manual template-string concat.
- **Hooks** live in `src/components/hooks/`. Shared types in `src/types.ts`.
- **Middleware** (`src/middleware.ts`) authenticates every request; gated paths come from the `PROTECTED_ROUTES` constant — edit there to gate new routes.

## Project

Mieszkanioholik — solo, after-hours, 8-week MVP for a personal apartment-hunting decision pipeline (paste-only, per-user rubric scoring, deal-breaker gate). Domain reference: @context/foundation/prd.md. Stack hand-off: @context/foundation/tech-stack.md. Bootstrap audit trail: @context/changes/bootstrap-verification/verification.md.

## Stack

`10x-astro-starter` — full stack identity in @context/foundation/tech-stack.md. Path alias: `@/*` → `./src/*` (use it instead of relative paths for any import crossing `src/` subdirectories).

## Daily commands

Two non-obvious caveats on top of @package.json scripts:

- Run `npx astro sync` after changing env types (or before `npm run build` in CI).
- `npx astro check` (type check) is separate from `npm run lint` — lint won't catch type errors.

## First-run setup

Use the `/dev-setup` skill for the full walkthrough. Critical project-specific difference from @README.md — Cloudflare local dev reads `.dev.vars`, NOT `.env`:

```
cp .env.example .dev.vars
```

Everything else (Docker, `npx supabase init/start`, email-confirmation toggle in Studio) is in the README — `/dev-setup` packages it as a checked sequence.

## CI

`.github/workflows/ci.yml` runs `npx astro sync` before lint + build. Requires GitHub repo secrets `SUPABASE_URL` and `SUPABASE_KEY`.

## Repo state to remember

`CLAUDE.md.scaffold` at root is the starter authors' original CLAUDE.md, sidelined during bootstrap. Diff against this file's project section to recover any starter convention you might want.

## 10x-cli context architecture

`context/` is owned by the 10xDevs toolkit and consumed by lesson skills:

- `context/foundation/` — durable: PRD, tech-stack, lessons.
- `context/changes/` — active work: bootstrap-verification log, frames, plans, implementations.
- `context/archive/` — **immutable** (see DO NOT rules above).
